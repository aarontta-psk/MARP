...
public:
	...
	T const& kesimo(int k) const {
		//si llega a nullptr, es que no hay kesimo (vacio o no)
		if (k > nelems)
...
protected:
	...
	bool inserta(T const& e, Link& a) {
		...
		if (a == nullptr) { // se inserta el nuevo elemento e
			a = new TreeNode(e); //tam_i == 1
			++nelems;
			crece = true;
		}
		else if (menor(e, a->elem)) {
			crece = inserta(e, a->iz);
			//en caso de que se inserte en la izquierda, aumentamos el tam_i de la raiz en 1
			if (crece) {
				a->tam_i++;
				reequilibraDer(a);
			}
	...
	void rotaDer(Link& r2) {
		...
		//cambio de tam tras hacer la rotacion
	...
	T const& k_esimo(Link const& ar, int k) const {
		if (k > ar->tam_i) //mayor, a la derecha y restamos k por tam_i
			return k_esimo(ar->dr, k - ar->tam_i);
		else if (k < ar->tam_i) //menor, a la izquierda sin modificar k
			return k_esimo(ar->iz, k);
		else //igual, estamos en k
			return ar->elem;
	}
